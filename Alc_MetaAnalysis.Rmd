---
title: "Exposure to Digital Alcohol Marketing and Use: Meta-Analysis"
...
```{r Read data}
Meta <- read_excel("Alc_Meta_DATA.xlsx", 
    sheet = "Statistics")
```
...
```{r DVs}
#Past 30-day 
Meta.Past30days <- subset(Meta, FINAL_DV=='Past 30-day use')

#Binge Drinking
Meta.Binge <- subset(Meta, FINAL_DV=='Binge drinking')

#Lifetime
Meta.Lifetime <- subset(Meta, FINAL_DV=='Lifetime use')

#Susceptibility to use
Meta.SUS <- subset(Meta, FINAL_DV=='Susceptibility to use')
...
```{r Run function for variance component}
...
mlm.variance.distribution(full.past30)
res <-  var.comp(full.past30)
res
```
```{r Binge}
# Calculate Variance Distribution
mlm.variance.distribution(full.binge)
res <-  var.comp(full.binge)
res
```
```{r Susceptibility to Use}
# Calculate Variance Distribution
mlm.variance.distribution(full.SUS)
res <-  var.comp(full.SUS)
res
```
```{r Lifetime}
# Calculate Variance Distribution
mlm.variance.distribution(full.Lifetime)
res <-  var.comp(full.Lifetime)
res
```

## Comparing Level 2 and Level 3 Models
```{r Comparing level 2 and level 3 - Past30}
Past30.model.comp <- rma.mv(yi = OR, 
                     V = Variance, 
                     slab = Author,
                     data = Meta.Past30days,
                     random = ~ 1 | Study/es.id, 
...
summary(Past30.model.comp)

anova(Past30.model.comp, full.past30)
```
```{r Comparing level 2 and level 3 - Binge}
Binge.model.comp <- rma.mv(yi = OR, 
                     V = Variance, 
                     slab = Author,
                     data = Meta.Binge,
                     random = ~ 1 | Study/es.id, 
...
summary(Binge.model.comp )

anova(Binge.model.comp, full.binge)
```
```{r Comparing level 2 and level 3 - SUS}
...
```
```{r Comparing level 2 and level 3 - Lifetime}
Lifetime.model.comp <- rma.mv(yi = OR, 
                     V = Variance, 
                     slab = Author,
                     data = Meta.Lifetime,
                     random = ~ 1 | Study/es.id, 
...
summary(Lifetime.model.comp)

anova(Lifetime.model.comp, full.Lifetime)
```
...
forest(past30days.Use,
            ...
            col.by = "black",
            JAMA.pval = TRUE,
            leftcols = c("studlab", "Effect_Size"),
            leftlabs = c("Study", "Effect Size"),
            colgap.left = ".3inch")

```
```{r Binge and forest plot}
#Binge
Binge.Use <- metagen(TE = OR,
            seTE = seTE,
            lower = OR.L.CI,
            upper = OR.U.CI,
            studlab = Author,
...
# Quality Assessment
```{r Design Quality}
Design_Quality <- update(Total,
                         data=Meta,
                         subgroup=Design_Quality,
                         tau.common = FALSE,
                         id = NULL)

summary(Design_Quality)


```
```{r Sample Breadth}
Sampling <- update(Total,
                  data=Meta,
...
# Moderator Analyses in Meta
```{r Past 30 day use moderators}
#Engagement
Past30days.Engagement <- update(past30days.Use,
                            data=Meta.Past30days,
                            subgroup = Exposure_Engagement,
                            tau.common = FALSE,
                            id = NULL)

summary(Past30days.Engagement)

#Timeframe
Past30days.Timeframe <- update(past30days.Use,
                            data=Meta.Past30days,
                            subgroup = Exposure_Timeframe,
                            tau.common = FALSE,
                            id = NULL)

summary(Past30days.Timeframe)

#Platform
Past30days.platform <- update(past30days.Use,
                            data=Meta.Past30days,
                            subgroup = Platform_FINAL,
                            tau.common = FALSE,
                            id = NULL)

summary(Past30days.platform )

#Age
Past30days.age <- update(past30days.Use,
                            data=Meta.Past30days,
                            subgroup = Age_Final,
                            tau.common = FALSE,
                            id = NULL)

summary(Past30days.age)

```
```{r Binge moderators}
#Exposure Type
Binge.Engagement <- update(Binge.Use,
                            data=Meta.Binge,
                            subgroup = Exposure_Engagement,
                            tau.common = FALSE,
                            id = NULL)

summary(Binge.Engagement)

#Timeframe
Binge.Timeframe <- update(Binge.Use,
                            data=Meta.Binge,
                            subgroup = Exposure_Timeframe,
                            tau.common = FALSE,
                            id = NULL)

summary(Binge.Timeframe)

#Platform
Binge.platform <- update(Binge.Use,
                            data=Meta.Binge,
                            subgroup = Platform_FINAL,
                            tau.common = FALSE,
                            id = NULL)

summary(Binge.platform )

#Age
Binge.age <- update(Binge.Use,
                            data=Meta.Binge,
                            subgroup = Age_Final,
                            tau.common = FALSE,
                            id = NULL)
...
```
```{r Susceptibility moderators}
#Exposure Type
SUS.Engagement <- update(SUS,
                            data=Meta.SUS,
                            subgroup = Exposure_Engagement,
                            tau.common = FALSE,
                            id = NULL)

summary(SUS.Engagement)

#Timeframe
SUS.Timeframe <- update(SUS,
                            data=Meta.SUS,
                            subgroup = Exposure_Timeframe,
                            tau.common = FALSE,
                            id = NULL)
...
```
```{r Lifetime moderators}
#Exposure Type
Lifetime.Engagement <- update(Lifetime.Use,
                            data=Meta.Lifetime,
                            subgroup = Exposure_Engagement,
                            tau.common = FALSE,
                            id = NULL)

summary(Lifetime.Engagement)

#Timeframe
Lifetime.Timeframe <- update(Lifetime.Use,
                            data=Meta.Lifetime,
                            subgroup = Exposure_Timeframe,
                            tau.common = FALSE,
                            id = NULL)
...
#Platform
Lifetime.platform <- update(Lifetime.Use,
                            data=Meta.Lifetime,
                            subgroup = Platform_FINAL,
                            tau.common = FALSE,
                            id = NULL)
...
#Age
Lifetime.age <- update(Lifetime.Use,
                            data=Meta.Lifetime,
                            subgroup = Age_Final,
                            tau.common = FALSE,
                            id = NULL)

summary(Lifetime.age)

```
```{r Adjusted vs unadjusted bias}
#Overall
Overall.Bias<- update(Total,
                      data=Meta,
                      subgroup = Adjusted_OR,
                      tau.common = FALSE,
                      id = NULL)
...
#Past30
Past30.Bias<- update(past30days.Use,
                      data=Meta.Past30days,
                      subgroup = Adjusted_OR,
                      tau.common = FALSE,
                      id = NULL)
...
#Lifetime
Lifetime.Bias<- update(Lifetime.Use,
                  data=Meta.Lifetime,
                  subgroup = Adjusted_OR,
                  tau.common = FALSE,
                  id = NULL)

summary(Lifetime.Bias)
...
lmeta <- limitmeta(PubBias)

# Funnel with curve
funnel.limitmeta(lmeta, xlim = c(1, 3))

# Funnel with curve and shrunken study estimates
funnel.limitmeta(lmeta, xlim = c(1, 3), shrunken = TRUE)

# PCurve Analysis
##Run dmetar function above
pcurve(PubBias)
...
```{r Robins E}
# Load your data
rob_data <- read_excel("ROBINS_E_robvis_ready.xlsx")

# Add the missing domain with default value (e.g., "Not Applicable")
rob_data$`Bias due to deviations from intended exposures` <- "Not Applicable"

# Reorder columns to match robvis expectation
rob_data <- rob_data[, c("Study",
...
# Plot
rob_summary(rob_data, tool = "ROBINS-E")

# Generate the traffic light plot
Rob_plot <- rob_traffic_light(rob_data, tool = "ROBINS-E")

ggsave("ROBINS_E_Traffic_Light_TALL.png", 
       plot = Rob_plot,
       dpi = 300,
       width = 10,     # same width
       height = 14)    # taller height
